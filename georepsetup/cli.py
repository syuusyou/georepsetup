...
def compare_disk_sizes(args, slavehost):
    """
    ...
    with glustermount(slavehost, args.slavevol) as mnt:
        if get_number_of_files(mnt):
            if not args.force:
                cleanup(slavehost, args.slavevol, mnt)
                output_notok("{0}::{1} is not empty. Please delete existing "
                             ...
                             "files.".format(slavehost, args.slavevol))
            else:
...
def setup_georep():
    ...
    passwd_prompt_msg = ("Geo-replication session will be established "
                         "between {mastervol} and {slave}::{slavevol}\n"
    ...
    compare_disk_sizes(args, slavehost)
    ...
    pubfile = "{mastervol}_{slavevol}_common_secret.pem.pub".format(
        mastervol=args.mastervol, slavevol=args.slavevol)
    ...
    create_georep_session(args, slavehost)
...
def get_args():
    ...
    parser.add_argument("slave",
                        help="Slave, HOSTNAME or root@HOSTNAME "
                        "or user@HOSTNAME",
                        metavar="SLAVE")
